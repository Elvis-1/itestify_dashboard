...
function DraftTest({ all, setAll, uploaded, setUploaded, scheduled, setScheduled, draft, setDraft }) {
    ...
    async function fetchDraftVideo() {
        ...
        try {
            ...
            let response = await axios.get(
                'https://itestify-backend-38u1.onrender.com/testimonies/videos/?type=drafts', 
                { 
    ...
    const handleUploadEdit = async (id) => {
        ...
        try {
            ...
            const response = await axios.put(
                `https://itestify-backend-38u1.onrender.com/testimonies/videos/${testimonyId}/`,
                requestData,
                {
                    headers: {
                        "Authorization": `Bearer ${token}`,
    ...
    const handleDeleteDraft = async () => {
        ...
        try {
            let response = axios.delete(`https://itestify-backend-38u1.onrender.com/testimonies/videos/${draftDetails.id}/`,
            {
                headers:{
                    'Authorization': `Bearer ${token}`
                }
